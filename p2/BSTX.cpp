...
int BinarySearchTreeX<Comparable>::sum(BinaryNodeX<Comparable> *t) const
{
  if (t == NULL){
    ...
    return sum(t->left) + t->element  + sum(t->right);
  }
}
  ...
  const Comparable & possibleAncestor) const
{
  ...
  if(find(possibleAncestor) == ITEM_NOT_FOUND){
    return false;
  }
  BinaryNodeX<int> t(find(possibleAncestor, root)->element, find(possibleAncestor, root)->left, find(possibleAncestor, root)->right);
...
int BinarySearchTreeX<Comparable>::highestFull(BinaryNodeX<Comparable> *t) const
{
  if(t->left == NULL){
    ...
    int temp1 = highestFull(t->left);
    int temp2 = highestFull(t->right);
    if(temp1 < temp2){
      return 1 + temp1;
    }
    else{
...
{
  int highestFull = this->highestFull();
  int height = this->height();
  return height - highestFull;
}